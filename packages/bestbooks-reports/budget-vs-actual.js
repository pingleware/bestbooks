...
class BudgetVsActual extends BaseReport {
    constructor() {
        super();
    }
    ...
    async formatHtml(formattedData) {
        return format("budgetVsActual",formattedData);
    }

    async formatXml(contents) {
        return array2xml("budgetVsActual",contents);
    }
    ...
    async retrieveReportDataSync(startDate, endDate) {
        return await this.budgetVsActualSync(startDate, endDate);
    }
}
