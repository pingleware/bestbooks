...
class RetainedEarningsReport extends BaseReport {
    constructor() {
        super();
    }
    ...
    async formatArray(data, notes) {
        var _data = {
            ...
            previous_retained_earnings: Number(data[0].beginning_retained_earnings).toFixed(2),
            net_income: Number(data[0].net_income).toFixed(2),
            dividends_paid: Number(data[0].dividends_paid).toFixed(2),
            retained_earnings: Number(data[0].ending_retained_earnings).toFixed(2),
            notes: notes
        };
...
module.exports = RetainedEarningsReport;