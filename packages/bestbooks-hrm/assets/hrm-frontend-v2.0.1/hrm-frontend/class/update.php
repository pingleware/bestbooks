...
class HRM_Updates {
    ...
    const product_id = 'hrm-frontend';
    const option     = 'hrm_front_end_license';
    const slug       = 'hrm-front-end';
     ...
     */
    function admin_menu() {
        add_submenu_page( 'hr_management', __( 'Updates', 'hrm' ), __( 'Updates', 'hrm' ), 'activate_plugins', 'hrm_updates', array($this, 'plugin_update') );
    }
    ...
    function license_enter_notice() {
        ...
        ?>
        <div class="error">
            <p><?php printf( __( 'Please <a href="%s">enter</a> your <strong>HRM front-end</strong> plugin license key to get regular update and support.' ), admin_url( 'admin.php?page=hrm_updates' ) ); ?></p>
        </div>
        <?php
    }
    ...
    function check_update( $transient ) {
        ...
        if ( version_compare( $plugin_version, $remote_info->latest, '<' ) ) {
            ...
            $basefile = plugin_basename( dirname( dirname( __FILE__ ) ) . '/hrm-front.php' );
    ...
    function get_current_plugin_info() {
        ...
        $plugin_data = get_plugin_data( dirname( dirname( __FILE__ ) ) . '/hrm-front.php' );
    ...
    function get_info() {
        ...
        $response = wp_remote_post( self::base_url . '?action=hrm_frontend_update_check', $params );
        $update = wp_remote_retrieve_body( $response );
    ...
    function plugin_update() {
        ...
        <div class="wrap">
            ...
            <h2><?php _e( 'Front-End Plugin Activation', 'hrm' ); ?></h2>
...
function hrm_front_end_license_update() {
    return new HRM_Updates();
}



//https://www.facebook.com/tuntuni.pakhi.7549