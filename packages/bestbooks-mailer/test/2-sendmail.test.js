...
describe("send email", function(){
    before(function(){
        // Initialize the SMTP server or any other setup needed before tests
        // start_smtp_server("smtp.com", 587);
        start_smtp_server("localdomain", 587, function() {
            console.log("SMTP server started on " + ip.address() + ":587");
        });
    });
    after(function(){
    ...
    it("1. sending email",function(){
        ...
        SendEMail("sender@localdomain","receiver@localdomain","New Estimate",message,25,function(results){
            console.log(results);
            var status = {
                status: 'success',
                location: 'sent',
