const assert = require('assert');
    const {deferredExpense} = require('../index');
    const {
      Model,
    ...
    describe('deferredExpense Testing', () => {
      ...
      it('should verify deferredExpense does exist', () => {
        // Sample test
        assert.ok(deferredExpense, 'deferredExpense should be defined');
      });

      it("should create a deferred expense", async () => {
        await deferredExpense(date, "Test Deferred Expense", 1000);
      });
      ...
      it("should show the accounts table contents",async()=>{
        ...
        assert.strictEqual(result[0].name, 'Deferred Expense');
