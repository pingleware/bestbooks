const assert = require('assert');
    ...
    describe('mrr Testing', () => {
      ...
      it('should verify mrr does exist', () => {
        // Sample test
        assert.ok(mrr, 'mrr should be defined');
      });

      it('should record a recurring subscription', async () => {
        await mrr(date, "Test Subscription", 100);
      });
      ...
      it("should show the accounts table contents",async()=>{
        ...
        assert.strictEqual(result[0].name, 'Unerned Revenue');
        assert.strictEqual(result[0].type, 'Liability');
        assert.strictEqual(result[1].code, 100);
        assert.strictEqual(result[1].name, 'Accounts Receivable');
