const assert = require('assert');
    const {unearnedRevenue} = require('../index');
    const {
      Model,
    ...
    describe('unearnedRevenue Testing', () => {
      ...
      it('should verify unearnedRevenue does exist', () => {
        // Sample test
        assert.ok(unearnedRevenue, 'unearnedRevenue should be defined');
      });

      it("should record a job deposit of $50,000 as unearned revenue",async()=>{
        await unearnedRevenue(date,"deposit from job for work not performed",50000);
      })
        ...
        const expected = [
          {
            ...
            account_name: 'Cash',
            txdate: date,
            note: 'deposit from job for work not performed',
            ref: 1,
            debit: 0,
            credit: 50000,
            balance: 50000,
      ...
      it("should show the accounts table contents",async()=>{
        ...
        assert.strictEqual(result[1].name, 'Unearned Revenue');
