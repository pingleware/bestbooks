const assert = require('assert');
    const {uccLienWriteOff} = require('../index');
    const {
      Model,
    ...
    describe('uccLienWriteOff Testing', () => {
      ...
      it('should verify uccLienWriteOff does exist', () => {
        // Sample test
        assert.ok(uccLienWriteOff, 'uccLienWriteOff should be defined');
      });

      it("should write-off $10,000 of the UCC lien",async()=>{
        await uccLienWriteOff(date,"write-off",10000,"Account Receivable","202401879224");
      })
        ...
        const expected = [
          {
            ...
            txdate: date,
            note: 'write-off UCC Lien No: 202401879224',
            ref: 1,
            debit: 0,
            credit: 10000,
            balance: 10000,
      ...
      it("should show the accounts table contents",async()=>{
        ...
        assert.strictEqual(result[1].name, 'Allowance for Doubtful Accounts');
        assert.strictEqual(result[1].type, 'ContraAsset');
