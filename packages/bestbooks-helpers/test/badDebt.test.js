const assert = require('assert');
    ...
    describe('badDebt Testing', () => {
      ...
      it('should verify badDebt does exist', () => {
        // Sample test
        assert.ok(badDebt, 'badDebt should be defined');
      });

      it("should record bad debt for $1000", async () => {
        await badDebt(date, "Uncollectible Account", 1000);
      });
      ...
      it("should show the accounts table contents",async()=>{
        ...
        assert.strictEqual(result.length > 0,true);
        assert.strictEqual(result[0].name, 'Bad Debt');
        assert.strictEqual(result[0].type, 'Expense');
        assert.strictEqual(result[0].code, 400);
        assert.strictEqual(result[1].name, 'Account Receivable');
