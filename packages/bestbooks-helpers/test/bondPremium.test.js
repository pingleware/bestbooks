const assert = require('assert');
    ...
    describe('bondPremium Testing', () => {
      ...
      it('should calculate bond premium correctly',async () => {
        const faceValue = 100000;
        const bondRate = 9; // 9%
        const marketRate = 8; // 8%
        const yearsToMaturity = 5;
        const maturityDate = new Date();
        maturityDate.setFullYear(maturityDate.getFullYear() + yearsToMaturity);
 
        await bondPremium(date, "New Bond Issue", faceValue, bondRate, marketRate, maturityDate);
      });
      ...
      it("should show the accounts table contents",async()=>{
        ...
        assert.strictEqual(result[1].name, 'Bond Premium');
