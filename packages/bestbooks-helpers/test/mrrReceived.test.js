const assert = require('assert');
    const {mrrReceived} = require('../index');
    const {
      Model,
    ...
    describe('mrrReceived Testing', () => {
      ...
      it('should verify mrrReceived does exist', () => {
        // Sample test
        assert.ok(mrrReceived, 'mrrReceived should be defined');
      });

      it('should record recurring revenue received', async() => {
        await mrrReceived(date,"Revenue received",500);
      })
      ...
      it("should show the accounts table contents",async()=>{
        ...
        assert.strictEqual(result[0].name, 'Accounts Receivable');
