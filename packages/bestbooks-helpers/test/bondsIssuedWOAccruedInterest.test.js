const assert = require('assert');
    const {bondsIssuedWOAccruedInterest} = require('../index');
    const {
      Model,
    ...
    describe('bondsIssuedWOAccruedInterest Testing', () => {
      ...
      it('should verify bondsIssuedWOAccruedInterest does exist', () => {
        // Sample test
        assert.ok(bondsIssuedWOAccruedInterest, 'bondsIssuedWOAccruedInterest should be defined');
      });

      // TODO: insert specific test case for the helper function below
      it('should process bonds issued without accrued interest correctly',async () => {
        const faceValue = 100000;

        await bondsIssuedWOAccruedInterest(date, `Bonds Issued without Accrued Interest`, faceValue);
      });
        ...
        const expected = [
          {
            ...
            account_name: 'Cash',
            txdate: date,
            note: 'Bonds Issued without Accrued Interest',
            ref: 1,
            debit: 100000,
            credit: 0,
            balance: 100000,
