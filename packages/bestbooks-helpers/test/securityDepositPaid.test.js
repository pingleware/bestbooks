const assert = require('assert');
    const {securityDepositPaid} = require('../index');
    const {
      Model,
    ...
    describe('securityDepositPaid Testing', () => {
      ...
      it('should verify securityDepositPaid does exist', () => {
        // Sample test
        assert.ok(securityDepositPaid, 'securityDepositPaid should be defined');
      });

      it("should process a $100 security deposit",async()=>{
        await securityDepositPaid(date,"Security deposit",100.00);
      })
      ...
      it("should show the accounts table contents",async()=>{
        ...
        assert.strictEqual(result[1].name, 'Security Deposit');
