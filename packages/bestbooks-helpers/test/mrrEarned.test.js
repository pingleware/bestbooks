const assert = require('assert');
    ...
    describe('mrrEarned Testing', () => {
      ...
      it('should verify mrrEarned does exist', () => {
        // Sample test
        assert.ok(mrrEarned, 'mrrEarned should be defined');
      });

      it('should record the earnings from a recurring subscription', async () => {
        await mrrEarned(date,"Eanings from subscription", 500);
      })
        ...
        const expected = [
          {
            ...
            txdate: date,
            note: 'Eanings from subscription',
            ref: 1,
            debit: 0,
            credit: 500,
            balance: 500,
      ...
      it("should show the accounts table contents",async()=>{
        ...
        assert.strictEqual(result[0].name, 'Subscription Revenue');
