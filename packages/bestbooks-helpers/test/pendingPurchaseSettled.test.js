const assert = require('assert');
    const {pendingPurchaseSettled} = require('../index');
    const {
      Model,
    ...
    describe('pendingPurchaseSettled Testing', () => {
      ...
      it('should verify pendingPurchaseSettled does exist', () => {
        // Sample test
        assert.ok(pendingPurchaseSettled, 'pendingPurchaseSettled should be defined');
      });

      it("should settle pending purchases",async()=>{
        await pendingPurchaseSettled(date,"settling pending purchases",318);
      })
        ...
        const expected = [
          {
            ...
            txdate: date,
            note: 'settling pending purchases',
            ref: 1,
            debit: 318,
            credit: 0,
            balance: 318,
      ...
      it("should show the accounts table contents",async()=>{
        ...
        assert.strictEqual(result[0].name, 'Bank');
        assert.strictEqual(result[0].type, 'Bank');
