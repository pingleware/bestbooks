const assert = require('assert');
    const {recognizeDeferredExpense} = require('../index');
    const {
      Model,
    ...
    describe('recognizeDeferredExpense Testing', () => {
      ...
      it('should verify recognizeDeferredExpense does exist', () => {
        // Sample test
        assert.ok(recognizeDeferredExpense, 'recognizeDeferredExpense should be defined');
      });

      it("should recognized a deferred expense",async()=>{
        await recognizeDeferredExpense("Asset Account","Expense Account",date,"a deferred expense",318);
      })
      ...
      it("should show the accounts table contents",async()=>{
        ...
        assert.strictEqual(result[0].name, 'Asset Account');
        assert.strictEqual(result[0].type, 'Asset');
        assert.strictEqual(result[1].code, 400);
        assert.strictEqual(result[1].name, 'Expense Account');
