const assert = require('assert');
    ...
    describe('equity Testing', () => {
      ...
      it('should verify equity does exist', () => {
        // Sample test
        assert.ok(equity, 'equity should be defined');
      });

      it("should let company issue shares worth $5000",async()=>{
        await equity("Investor", date, "Issue shares", 5000);
      });

      it ("should buyback shares worth $2000",async()=>{
        await equity("Investor", date, "Buy back shares", -2000);
      }); 
      ...
      it("should show the accounts table contents",async()=>{
        ...
        assert.strictEqual(result[0].name, 'Investor');
