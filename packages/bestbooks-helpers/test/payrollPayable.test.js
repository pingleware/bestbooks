const assert = require('assert');
    const {payrollPayable} = require('../index');
    const {
      Model,
    ...
    describe('payrollPayable Testing', () => {
      ...
      it('should verify payrollPayable does exist', () => {
        // Sample test
        assert.ok(payrollPayable, 'payrollPayable should be defined');
      });

      it('should record a payroll transaction',async()=>{
        await payrollPayable(date,"Pay staff",12000);
      })
        ...
        const expected = [
          {
            ...
            office_id: 0,
            account_code: '200',
            account_name: 'Net Payroll Payable',
            txdate: date,
            note: 'Pay staff',
            ref: 1,
            debit: 0,
      ...
      it("should show the accounts table contents",async()=>{
        ...
        assert.strictEqual(result[1].name, 'Net Payroll Payable');
