...
describe("Report Class", async function(){
    ...
    it('should return correct debit and credit totals for each account', async function() {
        this.timeout(async () => {
            ...
            assert.strictEqual(rows[2].name, 'Revenue'); // Fix: name should be 'Revenue'
            assert.strictEqual(rows[2].type, 'Revenue'); // Fix: type should be 'Revenue'
            ...
            assert.strictEqual(rows[1].name, 'Expense'); // Fix: name should be 'Expense'
