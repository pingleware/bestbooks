...
async function performRevenueRecognitionCheck() {
    ...
    const rows = await model.querySync(`
        SELECT le.id, le.txdate, le.debit, le.credit, le.account_name
        FROM ledger le
    ...
    if (Array.isArray(rows)) {
        rows.forEach(entry => {
            ...
            if (!entryDate.isValid()) {
                // Skip or log invalid date entries
                return;
            }
            if (entryDate.isAfter(today)) {
                errors.push({
                    message: `Revenue entry for "${entry.account_name}" is posted in the future on ${entry.txdate}.`,
                    entry
                });
            }
        });
...
async function performExpenseRecognitionCheck() {
    ...
    rows.forEach(entry => {
        if (entry.debit <= 0) {
          errors.push({
            message: `Expense entry for "${entry.account_name}" is improperly recognized.`,
            entry
          });
        }
    });
...
async function performAssetValuationCheck() {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const rows = await model.querySync(`SELECT account_name, balance 
          FROM ledger le 
          JOIN accounts ac ON le.account_name = ac.name 
          WHERE ac.type = 'Asset'`);
...
async function performInventoryValuationCheck() {
  ...
  const rows = await model.querySync(`SELECT le.account_name, le.balance 
      FROM ledger le 
      JOIN accounts ac ON le.account_name = ac.name 
  ...
  async function performGoingConcernCheck() {
    ...
    try {
      // TODO: Implement checkOperatingCashFlow() bestbooks-core module 
      //       audit-goingConcernAssumption.js
      //const goingConcern = new GoingConcernAssumption();
      //errors = await goingConcern.performAuditChecks();
    } catch (error) {
  ...
  async function performSegregationOfDutiesCheck() {
    ...
    const result = await model.querySync(
        `SELECT action, u.user_name, u.role
        FROM ledger l
    ...
    const conflicts = [];
    Object.values(actionsPerformed).forEach(function(user){
  ...
  async function performFairPresentationCheck() {
    ...
    const invalidEntries = await model.querySync(
        `SELECT * FROM ledger
        WHERE debit IS NULL OR debit <= 0 OR credit IS NULL OR credit <= 0`
    );
  ...
  // 14. Estimates and Assumptions - TODO: Implement this check
  /**
   * The Estimates and Assumptions principle in accounting refers to the use of estimates 
   * and assumptions in preparing financial statements. This is necessary when exact values 
   * cannot be determined, such as estimating bad debts, useful life of assets, or warranty costs.
   * 
   * To implement this check, we will query the ledger for entries marked as estimates 
   * and verify if they are properly documented.
   * 
   * @returns json { check: 'Estimates and Assumptions', errors }
   */
  
  // 15. Foreign Currency Transactions - TODO: Implement this check
  // This check would typically involve verifying that foreign currency transactions are recorded
  ...
  async function performLeasesAndIncomeTaxesCheck() {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const rows = await model.querySync(`SELECT * 
            FROM ledger 
            WHERE account_name LIKE "%lease%" AND debit > 0 OR credit > 0`);

        rows.forEach(entry => {
          if (entry.account_name) {
            const accountType = Ledger.findAccountType(entry.account_name);
            errors.push({
              message: `Lease transaction found for "${entry.account_name}" of type "${accountType}"`
            });
          }
          if (entry.debit > 0) {
            errors.push({
              message: `Income tax recorded for "${entry.account_name}" with tax amount of ${entry.debit}`
            });
          }
          if (entry.credit > 0) {
            errors.push({
              message: `Income tax credit recorded for "${entry.account_name}" with tax amount of ${entry.credit}`
            });
          }
        });
